...
Inventory::Inventory(const std::string& filename, const std::string& tempFilename) 
...
void Inventory::addItem() {
    int id;
    if (!getValidInput("Enter item ID: ", id)) return;
    ...
    int quantity;
    if (!getValidInput("Enter quantity: ", quantity)) return;

    double cost;
    if (!getValidInput("Enter cost: ", cost)) return;
...
void Inventory::updateItem() {
    ...
    if (findItem(idToUpdate, foundItem, position)) {
        ...
        int quantity;
        if (!getValidInput("Enter new quantity: ", quantity)) return;
        
        double cost;
        if (!getValidInput("Enter new cost: ", cost)) return;
